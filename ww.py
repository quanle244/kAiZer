...
def wagner_whitin(demand, purchase_cost, holding_cost, weights):
    num_products, num_months = demand.shape # Extract number of products and number of months from 'demand' (num_products = numner of columns = 30, num_months = number of rows = 12) 
    ...
    for product in range(num_products):
        ...
        order_cost_per_order = get_order_cost(weight) # After getting weight of the product, pass that weight into get_order_cost() function and the resulting ordering cost is stored in 'order_cost_per_order'.

        # Cost matrix for dynamic programming
        total_cost = np.full((num_months, num_months), float('inf')) # Create a 'total_cost' Matrix with dimensions(num_months, num_months) fully filled by every elements is initialized to infinity (float('inf')).

        # Precompute costs for ordering at month `start` to cover demand until month `end`.
        for start in range(num_months): # Iterates through possible 'start' months for placing an order.
            cumulative_demand = 0 # The total demand from month 'start' to month 'end'.
            for end in range(start, num_months): # Iterate through possible 'end' month to fulfill order starting from 'start' month.
               ...
               #Another way of Holding Cost Calculation  
               #  holding_cost_accum = sum(
               #     holding_cost[product,  k] * sum(demand[product, k + 1:end + 1])
        ...
        # dp[t] represents the Minimum cost to satisfy demnd from month 0 to month t.
        
        dp[0] = total_cost[0, 0] # Set the minimum cost meeting demand for the first month is the total cost of the that month. 
        # Create a 'backtrack' list with 'num_months' elements, all initialized to '-1' , '-1' represents for position not assigned yet.
        backtrack = [-1] * num_months  # For reconstructing the solution

        for t in range(1, num_months): # t : the current month that we evaluate for minimum cost.
            for s in range(t + 1): # s : start month of ordering periods that ends at month t (run from month 0 to month t)
                # cost : total cost from month s to month t.
                # cost = Min cost up to month (s-1) + total cost from month 's' to month 't'
                cost = (dp[s - 1] if s > 0 else 0) + total_cost[s, t] # s = 0 : there is no previous cost being considered, so cost = total_cost[0,t] 
        ...
        order_plan = np.zeros(num_months, dtype=int) # Create a array contains 12 zero values
        t = num_months - 1 # Start the backtracking process from the last month (t=11 for 12-month plan)
        while t >= 0: # The loop runs until t=0
